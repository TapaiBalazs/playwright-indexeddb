...
export type PlaywrightIDbStore = {
  createItem: <T>(key: (string | number), value: T) => Promise<PlaywrightIDbStore>,
...
export class Idbhelper {
  ...
  readItem<T>(store: string, key: string | number): Promise<T> {
    return this.makeCreateReadUpdateDeleteRequest<T>('get', store, key)
  }

  createItem<T>(store: string, key: string | number, value: T): Promise<PlaywrightIDbStore> {
    return this.makeCreateReadUpdateDeleteRequest<T>('add', store, key, value);
  }

  addItem(store: string, value: unknown): Promise<PlaywrightIDbStore> {
    return this.makeCreateReadUpdateDeleteRequest('add', store, null, value);
  }

  updateItem(store: string, key: string | number, value: unknown): Promise<PlaywrightIDbStore> {
    return this.makeCreateReadUpdateDeleteRequest('update', store, key, value);
  }
  ...
  store(storeName: string): PlaywrightIDbStore {
    ...
    return {
      createItem: <T>(key: string | number, value: T) => this.createItem<T>(storeName, key, value),
  ...
  private async makeCreateReadUpdateDeleteRequest<T>(
    ...
    const dbOperationResult = await this.page.evaluate(
      async ({ dbName, store, storeKey, storeValue, dbOperation }): Promise<T | null> => {
        const request = window.indexedDB.open(dbName);
        ...
        // @ts-expect-error
        const commandArguments: [T, string | number] | [string | number] | [T] = storeKey
          ? storeValue
            ? [storeValue, storeKey]
            : [storeKey]
          : [storeValue]
